  ...
  if (myString.length() >0) {
    ...
    switch(controllerType){
      ...
      case CONTROLLER_LIGHT_SENSOR:
        rawData = analogRead(pinNumber);
        processedData = (int) ((rawData / 1024f) * 100f);
        //float macarrones = (patatas / 1024) * 100;
        Serial.println("*"
                + convertToString(controllerId, 2) + 
                + "-" + convertToString(processedData, 4)
                + "-% ");
        break;
      default:
        break;
